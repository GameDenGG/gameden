...
DEFAULT_GAMES_PER_RUN = 600
DEFAULT_MIN_DELAY_SECONDS = 0.05
DEFAULT_MAX_DELAY_SECONDS = 0.20
DEFAULT_REQUEST_RETRIES = 2
DEFAULT_GAMES_PER_RUN_LIMIT = 1000

RAW_GAMES_PER_RUN = int(os.getenv("TRACK_GAMES_PER_RUN", DEFAULT_GAMES_PER_RUN))
TRACK_GAMES_PER_RUN_LIMIT = int(os.getenv("TRACK_GAMES_PER_RUN_LIMIT", DEFAULT_GAMES_PER_RUN_LIMIT))
if TRACK_GAMES_PER_RUN_LIMIT > 0:
    GAMES_PER_RUN = min(RAW_GAMES_PER_RUN, TRACK_GAMES_PER_RUN_LIMIT)
else:
    GAMES_PER_RUN = RAW_GAMES_PER_RUN
MIN_DELAY_SECONDS = float(os.getenv("TRACK_MIN_DELAY_SECONDS", DEFAULT_MIN_DELAY_SECONDS))
MAX_DELAY_SECONDS = float(os.getenv("TRACK_MAX_DELAY_SECONDS", DEFAULT_MAX_DELAY_SECONDS))
REQUEST_RETRIES = int(os.getenv("TRACK_REQUEST_RETRIES", DEFAULT_REQUEST_RETRIES))
TRACK_SHARD_TOTAL = int(os.getenv("TRACK_SHARD_TOTAL", "1"))
TRACK_SHARD_INDEX = int(os.getenv("TRACK_SHARD_INDEX", "0"))
ROLLOUT_HOLD_TIER = (os.getenv("TRACK_ROLLOUT_HOLD_TIER", "ROLLOUT_HOLD").strip().upper() or "ROLLOUT_HOLD")
TRACK_INCLUDE_ROLLOUT_HOLD = os.getenv("TRACK_INCLUDE_ROLLOUT_HOLD", "false").strip().lower() in {
    "1",
    "true",
    "yes",
    "on",
...
TRACK_HOT_MIN_PLAYERS = max(1, int(os.getenv("TRACK_HOT_MIN_PLAYERS", "7500")))
TRACK_MEDIUM_MIN_PLAYERS = max(1, int(os.getenv("TRACK_MEDIUM_MIN_PLAYERS", "1200")))

TRACK_HOT_REFRESH_MINUTES = max(5, int(os.getenv("TRACK_HOT_REFRESH_MINUTES", "20")))
TRACK_MEDIUM_REFRESH_MINUTES = max(30, int(os.getenv("TRACK_MEDIUM_REFRESH_MINUTES", "180")))
TRACK_COLD_REFRESH_MINUTES = max(120, int(os.getenv("TRACK_COLD_REFRESH_MINUTES", "1440")))
