...
HOMEPAGE_DEAL_CANDIDATE_POOL = max(
    HOMEPAGE_RAIL_LIMIT,
    int(os.getenv("HOMEPAGE_DEAL_CANDIDATE_POOL", "384")),
)
HOMEPAGE_DIVERSITY_WINDOW = 12
SNAPSHOT_CACHE_REBUILD_EVERY_BATCHES = max(1, int(os.getenv("SNAPSHOT_CACHE_REBUILD_EVERY_BATCHES", "3")))
RETRY_BACKOFF_BASE_SECONDS = max(1.0, float(os.getenv("SNAPSHOT_RETRY_BACKOFF_BASE_SECONDS", "30")))
RETRY_BACKOFF_MAX_SECONDS = max(
    RETRY_BACKOFF_BASE_SECONDS,
    float(os.getenv("SNAPSHOT_RETRY_BACKOFF_MAX_SECONDS", "3600")),
)
RETRY_BACKOFF_EXPONENT_CAP = max(1, int(os.getenv("SNAPSHOT_RETRY_BACKOFF_EXPONENT_CAP", "10")))
...
ALERT_DEDUPE_HOURS = max(1, int(os.getenv("ALERT_DEDUPE_HOURS", "6")))
DEAL_RADAR_LIMIT = max(10, int(os.getenv("DEAL_RADAR_LIMIT", "50")))
DEAL_RADAR_LOOKBACK_DAYS = max(1, int(os.getenv("DEAL_RADAR_LOOKBACK_DAYS", "3")))
DEAL_RADAR_ALERT_SCAN_LIMIT = max(
    DEAL_RADAR_LIMIT * 4,
    int(os.getenv("DEAL_RADAR_ALERT_SCAN_LIMIT", "320")),
)
DEAL_RADAR_TRENDING_POOL = max(
    DEAL_RADAR_LIMIT * 2,
    int(os.getenv("DEAL_RADAR_TRENDING_POOL", "120")),
)
DEAL_RADAR_POPULAR_POOL = max(
    DEAL_RADAR_LIMIT * 2,
    int(os.getenv("DEAL_RADAR_POPULAR_POOL", "120")),
)
DEAL_RADAR_DISCOUNT_POOL = max(
    DEAL_RADAR_LIMIT * 3,
    int(os.getenv("DEAL_RADAR_DISCOUNT_POOL", "200")),
)
DEAL_RADAR_MAX_PER_SIGNAL = max(3, int(os.getenv("DEAL_RADAR_MAX_PER_SIGNAL", "16")))
DEAL_RADAR_MAX_SIGNAL_SHARE = max(
    0.20,
    min(0.60, float(os.getenv("DEAL_RADAR_MAX_SIGNAL_SHARE", "0.30"))),
)
DEAL_RADAR_DIVERSITY_WINDOW = max(8, int(os.getenv("DEAL_RADAR_DIVERSITY_WINDOW", "16")))
DEAL_RADAR_MIN_SIGNAL_CATEGORIES = max(
    4,
    min(5, int(os.getenv("DEAL_RADAR_MIN_SIGNAL_CATEGORIES", "5"))),
)
DEAL_RADAR_BIG_DROP_MIN_ABS = max(0.5, float(os.getenv("DEAL_RADAR_BIG_DROP_MIN_ABS", "2.0")))
DEAL_RADAR_BIG_DROP_MIN_PCT = max(5.0, float(os.getenv("DEAL_RADAR_BIG_DROP_MIN_PCT", "15.0")))
...
ROLLOUT_HOLD_TIER = (os.getenv("TRACK_ROLLOUT_HOLD_TIER", "ROLLOUT_HOLD").strip().upper() or "ROLLOUT_HOLD")
